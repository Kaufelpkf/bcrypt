...
app.post('/logar',async(req,res)=>{
    ...
    const pesq = await Usuario.findOne({where: {email: login.email}, raw:true})
        ...
        if(pesq === null){
            ...
            res.status(200).json({message: "Usuário não existe no banco de dados!"})
        ...
        console.error("Erro ao consultar usuário no banco!")
        res.status(500).json({message: "Erro ao consultar usuário no banco!"})
    }    
